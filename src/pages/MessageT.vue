<template>
  <q-item to="/admin" color="primary" class="back">
    ←
  </q-item>
  <div v-if="message">
    <q-list separator>
  ...
  <q-scroll-area class="full-height flav absolute">
    <div class="q-py-lg q-px-md row items-center q-col-gutter-md">
      <div class="col col-5" :class="{ 'col-10': buf }">
        <q-input class="new-fayz" autogrow bottom-slots v-model="newFayzContent" placeholder="What do you need?" counter
          maxlength="280">
      ...
      <div class="col q-pr-md">
        <q-btn @click="addNewFayz" class="q-mr-md"
    ...
    <q-list>
      <transition-group v-for="fayz in fayzs" :key="fayz.id" appear enter-active-class="animated fadeIn slow"
        leave-active-class="animated fadeOut slow">
        <q-item v-if="fayz.num == String(routeParams.id)">
          <div class="fayz q-py-md">
            <q-item-section avatar top>
              <q-avatar size="xl">
...
setInterval(() => {
  localStorage.setItem('lastActiveM', Date.now());
}, 100);
...
const toggleLiked = async () => {
  if (message.value) {
    ...
    await updateDoc(likeRef, {
      liked: message.value.MuhammadAmin ? message.value.liked - 1 : message.value.liked + 1,
      MuhammadAmin: !message.value.MuhammadAmin
    });
  }
}
...
const toggleLikedtu = async (fayz) => {
  ...
  const newLikedValue = fayz.MuhammadAmin ? fayz.liked - 1 : fayz.liked + 1;
  const likeReff = doc(db, "fayzstu", fayz.id);
  await updateDoc(likeReff, {
    liked: newLikedValue,
    MuhammadAmin: !fayz.MuhammadAmin
  });
}
